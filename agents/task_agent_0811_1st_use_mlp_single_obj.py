...
class RealTimeSimAgent:
    ...
    def __init__(self, 
        ...
        ################optimizer################
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=0.01, betas=(0.9, 0.999))   #learning_rate
        ...
        for epoch in range(self.max_epoch):   #总的epoch
            ...
            for batch_idx in range(self.train_num_mini_batches): #每个epoch中的batch
                ...
                for buf_idx in range(self.buffer_num_per_batch):  #每个batch中的buffer，因为一个buffer是通过一次对环境的拍摄得到的
                    ...
                    # print(agnostic_high_idx)
                    # print(grasp_poses.shape, agnostic_high_idx.shape, sample_points.shape)
                    ...
                    for iter in range(self.iteration_per_buffer):
                        ...
                        #
                        per_step_points = sample_points[iter * self.model_cfg["num_env"]: (iter + 1) * self.model_cfg["num_env"], ...]
                        per_step_force = (per_step_points[:, 1] * 100).unsqueeze(-1)
                    ...
                    # batch_score_label[buf_idx, agnostic_high_idx, :] = self.force_buffer.all_label()   #(Num_grasp, 1) 50 x 1 ####0811保留
